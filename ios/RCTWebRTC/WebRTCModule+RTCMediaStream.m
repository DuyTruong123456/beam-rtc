...
RCT_EXPORT_METHOD(getDisplayMedia : (RCTPromiseResolveBlock)resolve rejecter : (RCTPromiseRejectBlock)reject) {
    ...
    NSLog(@"Audioconstraint",audioConstraints);
    if (videoTrack == nil) {
    ...
    if (videoTrack) {
        [mediaStream addVideoTrack:videoTrack];
        NSString *videoTrackId = videoTrack.trackId;
        self.localTracks[videoTrackId] = videoTrack;

        NSDictionary *videoTrackInfo = @{
